...
- [ ] Arm GN is full MIL-CREDA identical to G except that its normalization layers also learn from target; the rung G to GN reads what that normalization adds, so no gain is credited to the adaptation loss that belongs to normalization. `test_target_embeddings_updates_running_stats_for_gn`
- [ ] In every adapted arm except GN, unlabelled target passes through the encoder for the adaptation terms, but every layer with running statistics learns from source only. `test_running_stats_after_a_step_match_a_source_only_forward`
