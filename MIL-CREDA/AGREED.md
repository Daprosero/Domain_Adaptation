...
- [x] 2. A ceiling record exists at the scale the search declares for itself, carrying the six searched dimensions per transfer. Below that scale the search is a rehearsal: the ramp saturates by the second epoch and no value it chose is quotable. This is leveled because the record either meets its own declared scale or it does not. `@record:level ceilings`
- [ ] 3. The search's own report is current against this source, so what it says about how each configuration was chosen -- by a difference in the criterion, or by the rule inside a plateau -- describes the code that is here. Its rung is the rung of the record behind it: a report is only ever as trustworthy about scale as the record it read. `@notebook:level Notebooks/Benchmark_Search_Report_v1.ipynb`
- [ ] 4. The noise sweep ran and its notebook is current against this source. It sits here, between the search and the campaign, because it runs the floor alone under the values the search fixed, and every reading of the campaign afterwards is read beside its curve. Its rung is the rung of the run behind it: a sweep executed at pilot scale walks the same five levels with one seed, and what it draws is a shape rather than a measurement. `@notebook:level Notebooks/Benchmark_Noise_Sweep_v1.ipynb`
- [x] 5. The campaign ran and left evidence, at whichever rung it reached. Read this one knowing what it cannot see: a campaign run here at pilot scale leaves no shard at all, so this reads as the floor even when a pilot did run. That is a gap in the witness, not in the campaign. `@shard:level s00`
- [ ] 6. The attention mechanisms ran and left `attention_mechanisms.json`: the full method against ABMIL as published, ABMIL gated, max pooling and mean pooling, on the same configuration. Two-state because this step declares no notebook and no shard -- what says it ran is its own ledger entry, and a ledger entry carries no scale. `@step mechanisms`
- [ ] 7. The results notebook is current against this source and reads the merged record rather than a rehearsal's leftovers. This is the step the whole position exists for: a report executed against a pilot record once printed sixty runs beside an eighteen-hundred-run record with every other check green, and the rung is what makes that visible without anyone having to remember to doubt it. `@notebook:level Notebooks/Results_v1.ipynb`
<!-- /position -->
