# Agreed for the benchmark report — bound to research-concept-r21.md
...
- [ ] The sweep shows the whole curve over the five levels, so it chooses nothing: accuracy against rho for the floor, in source and in target. The instrument already exists and is already recorded; only the axis is new.
- [ ] The searched values are found at rho 0 and held fixed across all five levels. The curve is declared as the configuration chosen clean, applied dirty -- which is also the practical situation, since nobody recalibrates per noise level. Searching per level would multiply 6 transfers x 30 trials x 20 epochs by five, another whole campaign before the campaign. `test_el_barrido_lee_los_techos_una_vez_y_los_mantiene_en_los_cinco_niveles`
...
**"The degradation notebook shows the whole curve over the five levels, so it chooses nothing. Accuracy against rho for every arm, and `adaptationShare` against rho for every arm that carries an adaptation term. Both instruments already exist and are already recorded: no new measurement is needed, only the axis."** The sweep is the floor alone, and a floor carries no adaptation term: there is no share series left to draw against rho.
...
<!-- position revision=research-concept-r21.md sha256=cc55237fd6f7d9c6418a66671b64b87573cb5103465b31894fa1f1dc460178c2 derivedAt=2026-09-18T06:57:27Z session=52829b86 target=none -->
...
- [x] 2. The hyperparameter search ran and left a record at the scale it declares for itself, carrying the six searched dimensions per transfer. Below that scale the search is a rehearsal: the ramp saturates by the second epoch and no value it chose is quotable. Leveled because the record either meets its own declared scale or it does not. `@record:level ceilings`
- [x] 3. The noise sweep ran and its notebook is current against this source. It sits here, between the search and the campaign, because it runs the floor alone under the values the search fixed, and every reading of the campaign afterwards is read beside its curve. Its rung is the rung of the run behind it: a sweep executed at pilot scale walks the same five levels with one seed, and what it draws is a shape rather than a measurement. `@notebook:level Notebooks/Benchmark_Noise_Sweep.ipynb`
- [x] 4. The campaign ran and left evidence, at whichever rung it reached. The witness is the arrived shard and not the notebook, because a notebook that finished says the cells ran while a shard says the run returned with its stamp. Read it knowing what it cannot see: a campaign run here at pilot scale leaves no shard at all, so this reads as the floor even when a pilot did run. That is a gap in the witness, not in the campaign. `@shard:level s00`
- [x] 5. The attention mechanisms ran and their notebook is current against this source: the full method against ABMIL as published, ABMIL gated, max pooling at the instance level and mean pooling, on one configuration. Its rung is the rung of the run behind it. `@notebook:level Notebooks/Benchmark_Attention_Mechanisms.ipynb`
- [ ] 6. The results notebook is current against this source and reads the merged record rather than a rehearsal's leftovers. This is the step the whole position exists for: a report executed against a pilot record once printed sixty runs beside an eighteen-hundred-run record with every other check green, and the rung is what makes that visible without anyone having to remember to doubt it. `@notebook:level Notebooks/Benchmark_Results.ipynb`
<!-- /position -->
