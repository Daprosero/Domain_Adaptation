...
def _median_seeds(rate: float = 0.0, pilot: bool = False) -> dict | None:
    ...
    """
    # `kind` explicito y no por omision: `models_for` lleva tres coordenadas y
    # una llamada que pasa dos deja la tercera decidida por la firma, que es
    # exactamente como el barrido termino leyendo el arbol de la campana.
    record = config.models_for(rate, "campaign", pilot) / PROMOTION_RECORD
...
def available(rate: float = 0.0, pilot: bool = False) -> list[dict]:
    ...
    *dependant* arms' accuracy orderings, not by its own, so they are a biased
    ...
    for manifest_path in sorted(
            config.models_for(rate, "campaign", pilot).glob("*.manifest.json")):
