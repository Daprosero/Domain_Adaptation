...
def _bags(rng: np.random.Generator, count: int, dimension: int, shift: float, spread: float):
    bags = []
...
def _sharpened(rng: np.random.Generator, rows: int, n_classes: int, sharpness: float):
    """Class scores of controlled concentration, from uniform to near one-hot."""
    logits = rng.normal(size=(rows, n_classes)) * sharpness
...
def configuration(index: int) -> dict:
    ...
    K_s_c, K_st_c, K_t_c = conditional_blocks(
        K_ss, K_st, K_tt, np.arange(n_source), np.arange(n_target)
    )
    ...
    for column, (H_t, w_t) in enumerate(target):
        cross = K_st[:, column]
        pi = class_correspondence(cross, np.arange(n_source), tau)
    ...
    return {
        ...
        "squared_distances": np.asarray(squared_distances, dtype=float),
