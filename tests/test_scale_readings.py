...
class TestUnEnsayoNoSePuedeLeerComoUnaCorridaCompleta:
    ...
    # has no successor that renders a ceiling table: `Benchmark_Results.ipynb`
...
class TestElCuadernoLatenteLeeLosPesosDeSuPropiaCorrida:
    ...
    # analysis folds into `Benchmark_Results.ipynb`, which is not this stretch's
    ...
    # working this change in parallel). Measured directly: `Benchmark_Results.ipynb`
...
class TestQuienSabeSuEscalaLaDice:
    ...
    def test_ningun_cuaderno_se_escribe_su_escala_a_mano(self):
        ...
        `noise-report` se fusionaron en `Benchmark_Results.ipynb`, y los dos
        ...
        config.is_pilot_scale()`, y `Benchmark_Results.ipynb`'s
        ...
        `Benchmark_Results.ipynb` deriva la suya por desempaquetado -- sin este
