...
def test_local_distance_bounded_by_two(rng: np.random.Generator) -> None:
    ...
    far_apart = local_distance(1.0, np.array([0.0]), np.array([[1.0]]), np.array([1.0]))
    assert np.isclose(far_apart, 2.0, atol=TOL), "the adversarial fixture did not reach the bound"
    assert np.isclose(normalized_local_distance(far_apart), 1.0, atol=TOL)
    assert np.isclose(normalized_local_distance(1.0), 0.5, atol=TOL)
...
def test_source_loss_non_negative(rng: np.random.Generator) -> None:
    ...
    for _ in range(30):
        G = simplex_rows(rng, n_bags, C)
        Y = np.eye(C)[rng.integers(0, C, size=n_bags)]
        assert source_loss(G, Y) >= -1e-15

    certain = np.zeros((1, C))
    certain[0, 1] = 1.0
    assert certain.max() == 1.0, "the fixture is not a certain prediction"
    assert np.isclose(source_loss(certain, certain), 0.0, atol=1e-15)
    # The stabilizer still keeps the other extreme finite.
    wrong = np.zeros((1, C))
    wrong[0, 0] = 1.0
    assert np.isfinite(source_loss(certain, wrong))
